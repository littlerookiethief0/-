...
class Program:
    ...
    def upload(self, dir_name, dir_path, account, index):
        ...
        # 贴纸挑战
        options = ['流量卡推荐', '流量卡', '流量卡怎么选', '流量卡避坑指南', '流量卡官方正品', '流量卡测评']
        option=random.choice(options)
        self.page.get_by_text("点击输入挑战主题").click()
        self.page.locator('//input[@type="text"]').fill(option)
        self.page.locator(f'//div[@class="select-left--WCcCV"]//span[text()="{option}"]').click()
        # 选择音乐
        self.page.locator('//span[text()="选择音乐"]').click()
        self.page.locator('//input[@placeholder="搜索音乐"]').fill('young for you\n')
    ...
    def __del__(self):
        ...
        # 不加再次运行报错Event loop is closed! Is Playwright already stopped?
        if self.p: self.context.close() or self.p.stop()
    ...
    def run(self):
        ...
        for cookie_file in accounts:
            ...
            for index, (dir_name, dir_path) in enumerate(file_dirs.items(), start=1):
                if index > 222: break  # 每个账号每天上传有限制,只能上传75个
                try:
